...
const getMachineFriendlyId = async () => {
  const machineFriendlyId = await NetWorkConfigDB.getNetworkConfig(
    ObjectNameDB.MACHINE_MACHINE
  );

  if (machineFriendlyId.length > 0) {
    return machineFriendlyId[0].friendlyId;
  }

  return null;
};

//* When device's auth process is started, save device's friendlyId along with machineId sent for auth process
const setMachineIds = async ({ data }) => {
  const { machineID, friendlyId } = data;

  const machineData = { id: machineID, friendlyId: friendlyId };
