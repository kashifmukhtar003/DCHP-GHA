...
const Authorize = (props) => {
  ...
  const [machineFriendlyId, setMachineFriendlyId] = useState('');
  ...
  const getMachineId = async () => {
    const response = await window.api.send('osConfig', { action: 'machineId' });
    return response;
  };

  const getMachineFriendlyId = async () => {
    const response = await window.api.send('osConfig', {
      action: 'machineFriendlyId',
    });
    if (response) {
      setMachineFriendlyId(response);
    }
  };
  ...
  const displayMachineFriendlyId = () => {
    if (machineFriendlyId) {
      return (
        <p className="mt-0 mb-0 text-lg" key={machineFriendlyId}>
          This player has ID: {machineFriendlyId}
        </p>
      );
    }
    return null;
  ...
  const renderElements = () => {
    if (!authorization?.statusCode) {
      ...
      authorization.statusCode == 404 || Object.keys(props).length > 0
        ? !props?.isScreenAttached
        : false
    ) {
      return (
        <div className="h-full w-full bg-black text-white flex justify-center items-center h-full w-full relative flex-col">
